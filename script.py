START_TXT = """<b>Hᴇʏ {},
I'ᴍ ᴀ ғᴜɴ ʙᴏᴛ ᴍᴀᴅᴇ ʙʏ <a href='https://t.me/creatorbeatz'>Jᴏᴇʟ Kᴜʀɪᴀɴ Bɪɪᴜ</a>.

Cʟɪᴄᴋ ᴏɴ ᴛʜᴇ ℹ️ ʜᴇʟᴘ ʙᴜᴛᴛᴏɴ ᴀɴᴅ sᴇᴇ ᴍʏ ᴘᴏᴡᴇʀs 🔥

Mᴏʀᴇ ғᴇᴀᴛᴜʀᴇs ᴀʀᴇ ᴏɴ ᴛʜᴇ ᴡᴀʏ ✨</b>"""

HELP_TXT = """<b>👋 Hᴇʏ {},
Hᴇʀᴇ ᴀʀᴇ ᴍʏ ᴄᴏᴍᴍᴀɴᴅs...

/howilook - Sʜᴏᴡ ʏᴏᴜʀ ᴘɪᴄs 😂
/trash - Usᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ᴀs ᴀ ʀᴇᴘʟʏ ᴛᴏ ᴀɴʏ ᴍᴇssᴀɢᴇ. Oɴʟʏ ᴡᴏʀᴋs ɪɴ ɢʀᴏᴜᴘs. Tʜɪs ᴄᴏᴍᴍᴀɴᴅ ɪs ᴜsᴇᴅ ᴛᴏ ᴅᴇʟᴇᴛᴇ ᴀ sᴘᴇᴄɪғɪᴄ ᴍᴇssᴀɢᴇ ɪɴ ᴀ ɢʀᴏᴜᴘ.🗑️
/fun - Fᴏʀ ᴍᴏʀᴇ ғᴜɴ 💥

Mᴏʀᴇ ғᴇᴀᴛᴜʀᴇs ᴀʀᴇ ᴏɴ ᴛʜᴇ ᴡᴀʏ ✨
Bᴏᴛ ᴍᴀᴅᴇ ʙʏ <a href='https://t.me/creatorbeatz'><i>Jᴏᴇʟ ᴋᴜʀɪᴀɴ ʙɪɪᴜ</i></a></b>"""

LOOK_TXT = """<b>Tʜɪs ɪs ʜᴏᴡ {} Lᴏᴏᴋs ʟɪᴋᴇ 😂☝️

Mᴏʀᴇ ғᴇᴀᴛᴜʀᴇs ᴀʀᴇ ᴏɴ ᴛʜᴇ ᴡᴀʏ ✨</b>"""

ABOUT_TXT = """<b>Hᴇʏ {},
Tʜᴇsᴇ ᴀʀᴇ ᴍʏ ᴅᴇᴛᴀɪʟs...
✯ 𝙼𝚈 𝙽𝙰𝙼𝙴: 𝚃𝙷𝙴 𝙵𝚄𝙽 𝙱𝙾𝚃
✯ 𝙲𝚁𝙴𝙰𝚃𝙾𝚁: <a href=https://t.me/creatorbeatz>𝙹𝙾𝙴𝙻 𝙺𝚄𝚁𝙸𝙰𝙽 𝙱𝙸𝙹𝚄</a>
✯ 𝙻𝙸𝙱𝚁𝙰𝚁𝚈: 𝙿𝚈𝚁𝙾𝙶𝚁𝙰𝙼
✯ 𝙻𝙰𝙽𝙶𝚄𝙰𝙶𝙴: 𝙿𝚈𝚃𝙷𝙾𝙽 𝟹
✯ 𝙱𝙾𝚃 𝚂𝙴𝚁𝚅𝙴𝚁: 𝙾𝙺𝚃𝙴𝚃𝙾
✯ 𝙱𝚄𝙸𝙻𝙳 𝚂𝚃𝙰𝚃𝚄𝚂: v1.0.1 [ 𝙱𝙴𝚃𝙰 ]</b>"""

SOURCE_TXT = """<b>Nᴏᴛᴇ:-
- Tʜɪs Bᴏᴛ Is A Cʟᴏsᴇ Sᴏᴜʀᴄᴇ Pʀᴏɪᴇᴄᴛ.
- Dᴇᴠᴇʟᴏᴘᴇʀ - <a href='t.me/creatorbeatz'>Jᴏᴇʟ ᴋᴜʀɪᴀɴ ʙɪɪᴜ</a></b>"""

OWNER_INFO = """
<b>⍟───[ ᴏᴡɴᴇʀ ᴅᴇᴛᴀɪʟꜱ ]───⍟
    
• ꜰᴜʟʟ ɴᴀᴍᴇ : ᴊᴏᴇʟ ᴋᴜʀɪᴀɴ ʙɪᴊᴜ
• ᴜꜱᴇʀɴᴀᴍᴇ : @creatorbeatz
• ᴘᴇʀᴍᴀɴᴇɴᴛ ᴅᴍ ʟɪɴᴋ : <a href='t.me/creatorbeatz'>ᴄʟɪᴄᴋ ʜᴇʀᴇ</a></b>"""

MOVIE_MAL_TXT = """<b>ഹലോ {}, എന്തിനാണ് മൂവിയുടെ പേരിന്റെ കൂടെ "Movie" എന്ന് കൊടുക്കുന്നത്...🤦‍♂🤦‍♂

താഴെ കൊടുത്തിരിക്കുന്നത് പോലെ Request ചെയുക 😵‍💫👇

✅ Marakkar
✅ Marakkar 2021 

❌ Wrong Methods👇 ❌

❌ Marakkar Movie ❌</b>"""

MOVIE_ENG_TXT = """<b>Hello {}, Why do you type the word "Movie" along with the name of the film...

Request as given below 👇

✅ Annabelle
✅ Annabelle 2014 

❌ Wrong Methods👇 ❌

❌ Annabelle Movie ❌</b>"""

MV_TXT = """
<b>ᴊᴏɪɴ ᴏᴜʀ ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ ʙʏ ᴄʟɪᴄᴋɪɴɢ ᴛʜᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇

ᴅᴏɴ'ᴛ ᴀsᴋ ᴛʜᴇᴀᴛʀᴇ ᴘʀɪɴᴛs ᴡᴇ ᴡᴏɴ'ᴛ ᴜᴘʟᴏᴀᴅ ᴛʜᴇᴀᴛʀᴇ ᴘʀɪɴᴛs...

ᴀʟʟ ʟᴀɴɢᴜᴀɢᴇ ᴍᴏᴠɪᴇs ᴀᴠᴀɪʟᴀʙʟᴇ...

ᴏʟᴅ ᴀɴᴅ ɴᴇᴡ ᴍᴏᴠɪᴇs ᴀᴠᴀɪʟᴀʙʟᴇ...

• <a href='https://t.me/+jMyhIRhcp9EyZmJl'>ᴄʟɪᴄᴋ ᴍᴇ ᴛᴏ ᴊᴏɪɴ ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ</a>

• <a href='https://t.me/+GOFte-Rz2tcxODg1'>ᴄʟɪᴄᴋ ᴍᴇ ᴛᴏ ᴊᴏɪɴ sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ</a>

• <a href='https://t.me/+vu3FBEXifbRhNTk9'>ᴄʟɪᴄᴋ ᴍᴇ ᴛᴏ ᴊᴏɪɴ ʙᴀᴄᴋ-ᴜᴘ ᴄʜᴀɴɴᴇʟ</a></b>"""

KICKED = """
<b>Sorry {}, You are banned to use me 😔</b>"""

FSUB = """
<b>Hey {}, You need to join our updates channel to use this bot... 😌</b>"""

COMMAND_USER = """
<b>Hey {}, This is an Admin command, Not for you!</b>"""
